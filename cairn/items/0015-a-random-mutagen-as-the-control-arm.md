...
Code-review fixes (findings 1-5). flasks.new now writes the RESOLVED arm (c.mutagen_kind) into flasks.json, not 'mutagen or config.MUTAGEN_KIND': a malformed BIOTIC_MUTAGEN with no --mutagen flag falls back to 'mixed' at construction, so the manifest no longer records a raw invalid string while the flasks actually run 'mixed' (test_new_records_the_resolved_arm_not_a_bad_env). fossilize() maps origin 'llm' -> 'semantic' in the header prose only (stored origin unchanged), matching docs/mutagen.md (test_a_semantic_fossil_reads_semantic_not_llm). docs/curve.md now states the arisen_llm+arisen_random+1==arisen identity closes exactly only for a dish founded under this apparatus and while hgt is unused: pre-feature strains load with null origin and count in neither column, and the schema's hgt origin (item 0014) will too until an arisen_hgt column exists. Added test_mixed_at_the_shipped_share_produces_both_arms to cover the shipped mixed@0.25 mid-range split (both arms fire: 14 llm, 7 random over 6000 ticks), which the extremes (share 0.0/1.0) did not exercise.
